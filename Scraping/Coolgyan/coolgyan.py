...
url_base = 'https://coolgyan.org/ncert-solutions/ncert-solutions-class'
classes = [str(i) for i in range(9, 10)]
# https://coolgyan.org/ncert-solutions/ncert-solutions-class-9-social-science-history/
...
for cls in classes:
    ...
    for subject in subjects:
        ...
        if page.status_code == 404:
            print("Page " + cls + " " + subject + " Does Not Exist (404)")
            continue
        try:
                soup = BeautifulSoup(page.content, 'html.parser')  
        except:
            print("Page " + cls + " " + subject + "BS4 Error")
            continue
        # table_resp = soup.find('div',attrs={"class" : "table-responsive"})
        # print(table_resp.text)
        ...
        for row in rows:
            ...
            for cell in cells:
                ...
                article_content = chapter_soup.find('article',attrs={"id" : re.compile("post-[d]*")}).findAll(['p'])
                qs = chapter_soup.find('article',attrs={"id" : re.compile("post-[d]*")}).findAll(['strong'])
                for row in qs:
                    # content += row.text
                    # content += '\n'
                    print(row.text)
